...
async function requestDisplayPermission() {
  ...
  if (stream) {
    ...
    requestPermissionButton.children[1].innerHTML = tickIcon;
    enableButton(takeScreenshotButton);
    enableButton(videoMonitoringButton);
  }
}
...
function cropImage(imageUrl: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    image.onload = () => {
      ...
      const horizontalCrop = widthCropping;
      canvas.width = image.width - horizontalCrop * 2;
      // Reduce height by 200px to account for the bottom bottom bar and the url bar
      canvas.height = image.height - verticalCropping * 2;
      const context = canvas.getContext("2d");
      if (context) {
        context.drawImage(image, -horizontalCrop, -verticalCropping);
